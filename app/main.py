...
# --- STATE MANAGERS (Caching and Session States) ---
# Ensure variables survive redraw cycles
...
# If user types key in sidebar, update environment variables
...
with st.sidebar:
    ...
    if api_input != st.session_state["api_key_override"]:
        st.session_state["api_key_override"] = api_input
    ...
    chunk_size = st.slider(
        ...
        value=3000,
        step=500,
        help="Target size for mapping chunks. Smaller splits retain more granularity; larger values compile faster."
    )
    ...
    if not uploaded_file:
        ...
        <div class="glass-panel" style="text-align: center; padding: 40px;">
            ...
            <p style="color:#4b5563; max-width: 600px; margin: 15px auto;">
                To get started, drag-and-drop a PDF file into the <b>Upload PDF Document</b> area on the left control panel, select your target summary layout, and watch the Map-Reduce pipeline compile it live!
            </p>
            <div style="display:flex; justify-content:center; gap: 20px; margin-top:20px; flex-wrap: wrap;">
                ...
                <div class="metric-card" style="width: 130px;">
                    <div class="metric-val">💾</div>
                    <div class="metric-label" style="font-size:0.75rem;">SQLite History</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not parse_result["success"]:
            ...
            # Submit summaries pipeline trigger
            col_btn, col_info = st.columns([1, 2])
            with col_btn:
                start_btn = st.button("🚀 Generate Summarization Report", use_container_width=True, type="primary")
            with col_info:
                st.caption(f"Will segment text into blocks of ~{chunk_size} characters using '{summary_format}' layout.")
...
with tab_diagnostics:
    ...
    st.caption("Verify configuration parameters, database status, and Google Gemini API connectivity.")
    ...
    with col_d1:
        ...
        st.write(f"**Default Model Configuration:** `{model_name}`")
        if api_key and api_key != "your_gemini_api_key_here":
            st.success("🔑 API Key status: **Configured** (Valid variable found)")
        else:
            st.warning("⚠️ API Key status: **Not Configured** (Using placeholders. Please add key in sidebar or config/.env)")
            
        test_conn_btn = st.button("🔍 Run Ping test to Gemini API")
        if test_conn_btn:
            with st.spinner("Pinging API endpoint..."):
                ping_res = verify_api_connection()
            if ping_res["success"]:
